// piramidka1.cpp : main project file.
...
void RenderScene(void) {
	...
	matrixStack.LoadMatrix(viewFrustum.GetProjectionMatrix());
	matrixStack.MultMatrix(cameraMatrix);
	glUniformMatrix4fv(MVPMatrixLocation,1,GL_FALSE,matrixStack.GetMatrix());
	drawGrid(0.1);
	matrixStack.Rotate(angle*100, 0, 0, 1.0f);
	glUniformMatrix4fv(MVPMatrixLocation,1,GL_FALSE,matrixStack.GetMatrix());
	drawPyramid();
	matrixStack.Translate(-1.1f, -0.3f, 0);
	matrixStack.Rotate(angle*100, 0, 0, 1.0f);
